        ...
        {data.leagues.map((item: any) => (
          ...
          >
            <div className="flex items-center gap-4">
              <div className="bg-white dark:bg-black/20 p-3 rounded-xl shadow-inner">
                <Image src={item.league.logo} width={48} height={48} className="w-12 h-12 object-contain" alt="" />
              </div>
              <div className="flex-1">
                <h3 className="font-bold text-slate-800 dark:text-white group-hover:text-green-600 transition-colors">{item.league.name}</h3>
                <span className="text-xs text-slate-500 dark:text-gray-500 font-medium">{item.country.name}</span>
              </div>
