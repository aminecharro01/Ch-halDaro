...
export default function TeamPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: leagueData } = useSWR(team?.team?.leagueId ? `/api/league/${team.team.leagueId}` : null, fetcher);
    ...
    <div className="animate-fade-up max-w-7xl mx-auto p-4 space-y-8 pb-20">
      ...
      <div className="relative rounded-[2.5rem] overflow-hidden border border-white/10 shadow-2xl bg-gray-950">
        ...
        <div className="absolute inset-0 z-0">
          {team.team.banner ? (
            <>
              <img src={team.team.banner} alt="" className="w-full h-full object-cover opacity-30" />
            ...
            <div className="flex-1 text-center md:text-left space-y-6">
              <div className="space-y-2">
                <div className="flex items-center justify-center md:justify-start gap-3">
                  ...
                  <FavoriteButton itemId={id} itemType="team" itemName={team?.team?.name} itemLogo={team?.team?.logo} className="scale-150" />
                </div>
                ...
                <div className="flex flex-wrap items-center justify-center md:justify-start gap-6 text-sm font-bold text-gray-400 uppercase tracking-[0.2em]">
                  <span className="flex items-center gap-2"><MapPin className="w-4 h-4 text-green-500" /> {team.team.country}</span>
                  <span className="flex items-center gap-2"><Calendar className="w-4 h-4 text-blue-500" /> Est. {team.team.founded}</span>
              ...
              <div className="flex items-center justify-center md:justify-start gap-4">
                {team.team.social?.website && (
                  <a href={`https://${team.team.social.website}`} target="_blank" rel="noopener noreferrer" className="p-3 bg-white/5 rounded-2xl hover:bg-white/10 border border-white/5 transition-all text-white">
                    <Globe className="w-5 h-5" />
                  </a>
                )}
                {team.team.social?.facebook && (
                  <a href={`https://${team.team.social.facebook}`} target="_blank" rel="noopener noreferrer" className="p-3 bg-white/5 rounded-2xl hover:bg-white/10 border border-white/5 transition-all text-white">
                    <ExternalLink className="w-5 h-5" />
                  </a>
                )}
                {team.team.social?.twitter && (
                  <a href={`https://${team.team.social.twitter}`} target="_blank" rel="noopener noreferrer" className="p-3 bg-white/5 rounded-2xl hover:bg-white/10 border border-white/5 transition-all text-white">
                    <ExternalLink className="w-5 h-5" />
                  </a>
                )}
                {team.team.social?.instagram && (
                  <a href={`https://${team.team.social.instagram}`} target="_blank" rel="noopener noreferrer" className="p-3 bg-white/5 rounded-2xl hover:bg-white/10 border border-white/5 transition-all text-white">
                    <ExternalLink className="w-5 h-5" />
                  </a>
                )}
              </div>
            ...
            <div className="space-y-6 animate-fade-up">
              ...
              <div className="bg-gray-900/40 backdrop-blur-md border border-white/5 rounded-[2rem] p-8">
                <p className="text-gray-400 leading-relaxed text-sm md:text-base font-medium">
                  {team.team.description.length > 600 
                    ? team.team.description.substring(0, 600) + '...' 
                    : team.team.description}
                </p>
                {team.team.description.length > 600 && (
                  <button className="mt-4 text-green-500 text-sm font-bold uppercase tracking-widest hover:text-green-400 transition-colors">
                    Read more
                  </button>
                )}
              </div>
          ...
          {team.team.equipment && (
            <div className="bg-gray-900/40 backdrop-blur-md border border-white/10 rounded-[2rem] p-6 space-y-4 shadow-lg text-center">
              ...
              <div className="relative group">
                ...
                <img src={team.team.equipment} alt="Team Jersey" className="relative h-48 mx-auto object-contain drop-shadow-2xl transition-transform duration-500 group-hover:rotate-12" />
              </div>
            </div>
          )}
        </div>
