...
import { Goal, Square, AlertTriangle, Monitor, Timer, Mail, Send, Loader2 } from "lucide-react";
import { TEST_ALERT_KEYS, TEST_ALERT_DELAY_MS } from '@/lib/test-alerts';
    ...
    <div className="max-w-2xl mx-auto space-y-8 animate-fade-up">
          ...
          <button 
            onClick={async () => {
              ...
              try {
                ...
                setStatusMessage({
                  text: 'Sending 6 push notifications (~3s apart)… Enable email alerts and log in for emails.',
                  type: 'info',
                });
                ...
                for (let i = 0; i < TEST_ALERT_KEYS.length; i++) {
                  const alertKey = TEST_ALERT_KEYS[i];
                  setStatusMessage({
                    text: `Sending test ${i + 1}/${TEST_ALERT_KEYS.length}: ${alertKey}…`,
                    type: 'info',
                  });
                  ...
                  const res = await fetch('/api/test-notification', {
                    ...
                    body: JSON.stringify({
                      subscription,
                      alertKey,
                      includeEmail: prefs.email_alerts,
                    }),
                  });
                  const data = await res.json();
                  if (!res.ok) {
                  ...
                  totalPush += data.pushSent ?? 0;
                  totalEmail += data.emailsSent ?? 0;
                  if (data.emailSkipReason) emailHint = data.emailSkipReason;

                  if (i < TEST_ALERT_KEYS.length - 1) {
                    await new Promise((r) => setTimeout(r, TEST_ALERT_DELAY_MS));
                  }
                }
                ...
                const emailPart =
                  totalEmail > 0
                    ...
                    : emailHint
                      ? `0 emails (${emailHint})`
                      : '0 emails (enable Email alerts + RESEND_API_KEY)';

                setStatusMessage({
                  text: `✓ Done: ${totalPush} push + ${emailPart}.`,
                  type: totalPush > 0 ? 'success' : 'error',
                });
              } catch (err: unknown) {
            ...
            disabled={!isSubscribed || isTestingAll}
            className={`w-full sm:w-auto font-bold px-6 py-3 rounded-xl transition border border-gray-700 inline-flex items-center justify-center gap-2 ${isSubscribed && !isTestingAll ? 'bg-gray-800 text-white hover:bg-gray-700' : 'bg-gray-700/50 text-gray-400 cursor-not-allowed'}`}
          >
            {isTestingAll ? (
              <>
      ...
      <div className="space-y-6 bg-gray-900/40 border border-gray-800/60 p-6 rounded-3xl">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="flex items-center justify-between p-4 bg-gray-900 rounded-xl border border-gray-800 sm:col-span-2">
            <div>
              <div className="font-medium text-gray-200 flex items-center gap-2">
                <Mail className="w-4 h-4 text-sky-400" /> Email alerts
              </div>
              <div className="text-xs text-gray-500">Requires login — uses your account email</div>
            </div>
            <input
              type="checkbox"
          ...
          <div className="flex items-center justify-between p-4 bg-gray-900 rounded-xl border border-gray-800">
            <div>
              <div className="font-medium text-gray-200">Penalties</div>
          ...
          <div className="flex items-center justify-between p-4 bg-gray-900 rounded-xl border border-gray-800">
            <div>
              <div className="font-medium text-gray-200">VAR Reviews</div>
          ...
          <div className="flex items-center justify-between p-4 bg-gray-900 rounded-xl border border-gray-800">
            <div>
              <div className="font-medium text-gray-200">Match Status</div>
      ...
      <div className="bg-[#24A1DE]/10 border border-[#24A1DE]/30 p-6 rounded-3xl">
        <h2 className="font-bold text-white mb-2 flex items-center gap-2">
          <Send className="w-5 h-5" /> Telegram Bot
        </h2>
